...
def compute_control_input(desired_state, robot_state, dist_reading, obst_points):
    s = robot_state[:2] + np.array([l*np.cos(robot_state[2]), l*np.sin(robot_state[2])]) 
    u_gtg_s = desired_state[:2] - s
    rot_mat = np.matrix([[np.cos(robot_state[2]), np.sin(robot_state[2])],[-np.sin(robot_state[2]), np.cos(robot_state[2])]])
    l_mat = np.matrix([[1,0],[0,1/l]])
    u_gtg = l_mat @ rot_mat @ u_gtg_s
    v_control, omega, w_l, w_r = limit_wheel_speed(u_gtg[0,0], u_gtg[0,1])
    u_gtg = np.array([v_control, omega])
    rot_mat_inv = np.matrix([[np.cos(robot_state[2]), -np.sin(robot_state[2])],[np.sin(robot_state[2]), np.cos(robot_state[2])]])
    l_mat_inv = np.matrix([[1,0],[0,l]])
    u_gtg_s = rot_mat_inv @ l_mat_inv @ u_gtg
    ...
    if(len(obstacle_coords) == 0):
        ...
        u_s = compute_QP(s, u_gtg_s, obstacle_coords)
        current_input = l_mat @ rot_mat @ u_s
        current_input = np.array([current_input[0,0], current_input[0,1]])

    return current_input

def compute_QP(s, u_gtg_s, obstacle_coords):
    h = []
    ...
    c = -2 * cvxopt.matrix(u_gtg_s, tc = 'd')
    c = c.trans()
...
def simulate_control():
    ...
    for it in range(sim_iter):
        ...
        #------------------------------------------------------------
        current_input = compute_control_input(desired_state, robot_state, distance_reading, obst_points)
        #------------------------------------------------------------
