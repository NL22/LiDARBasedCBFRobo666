...
def compute_QP(robot_state, u_gtg, obstacle_coords):
    ...
    H= -2*(robot_state[:2] - obstacle_coords)
    h = compute_h(robot_state, obstacle_coords)
    b = 0.1*h**3
...
def compute_h(robot_state, obstacle_coords):
    #h = (robot_state[0] - obstacle_coords[0])**2 + (robot_state[1] - obstacle_coords[1])**2 - R_s**2
    dist = np.linalg.norm(robot_state[:2] - obstacle_coords)
    h = (dist)**2 - R_s**2
    #print(h)
    return h
