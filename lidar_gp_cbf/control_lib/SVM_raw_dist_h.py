...
class OnlineSVMModel():
    ...
    def set_new_data(self, new_X, new_Y=np.array([np.float64(1)]), theta=0.0, safe_offset=0.05):
        """
        ...
        if self.data_X is None:
            ...
            distance = new_X  # Calculate distance from the obstacle
            #label = label_function(distance, sense_dist, safe_offset)
            label=-1.0
            #self.data_X = new_feature
            self.data_X = np.array([[new_X, theta[0]]])
            self.data_Y = np.array([[-1.0]]) # Assign label based on distance
            self.iter = np.array([self.k])  # Initialize iteration tracking array
                ...
                self.data_X = np.append(self.data_X, [[new_X, theta[0]]], axis=0)
                ...
                pos_point = np.array([[new_X-safe_offset, theta[0]]])
                self.data_X = np.append(self.data_X, pos_point, axis=0)
                self.data_Y = np.append(self.data_Y, [[+1.0]])
                self.iter = np.append(self.iter, self.k)
    ...
    def update_SVG(self):
        ...
        if len(self.data_X) > 3:  # Ensure there are enough points before training
            ...
            self.regr.fit(self.data_X, self.data_Y.ravel())
    ...
    def get_cbf_safety_prediction(self, t):
        ...
        self.update_SVG()
        n_points = t.shape[0]  # Number of points in t
        
        # Initialize arrays to store results for all points
        hsvm_xq = np.zeros((n_points, 1))
        svm_G = np.zeros((n_points, 2))  # Gradient for each point (distance, angle -> 2D)
        svm_h = np.zeros((n_points, 1))  # Adjusted h values for each point

        # Loop through each point in t and compute h and gradient
        for i in range(n_points):
            point = t[i].reshape(1, -1)  # Extract the current (distance, angle) point

            # Get the actual value of h at this point
            hsvm_xq[i, 0] = self.get_h_value(point)[0]

            # Compute the gradient of RBF kernel with respect to the current point
            svm_G[i, :] = self.compute_gradient(point, sigma=2.0).flatten()

            # Compute the adjusted h value for safety constraints (subtract dh_dt)
            svm_h[i, 0] = hsvm_xq[i, 0] - self.dh_dt

        # Return the computed gradient, safety function value minus dh_dt, and h(x)
    ...
    def draw_gp_whole_map_prediction(self, ax, field_x, field_y, ic, robot_pos, sensing_rad, color='r'):
        ...
        if self.N>9: # Assign with 
            ...
            # Compute angles for each point in map_local (arctan2 gives the angle in radians)
            angles = np.arctan2(map_local[:, 1], map_local[:, 0])

            # Compute distances (norms) for each point
            distances = np.linalg.norm(map_local, axis=1)

            # Combine distances and angles into the input format for get_cbf_safety_prediction
            # We are combining them into a single array where each row contains [distance, angle] for each point
            distance_angle_input = np.column_stack((distances, angles))
            _,_,self.hpg_map=self.get_cbf_safety_prediction(distance_angle_input)
            x_values = self.data_X[:, 0] * np.cos(self.data_X[:, 1])
            y_values = self.data_X[:, 0] * np.sin(self.data_X[:, 1])

            # Update the plot by adding the robot's position to these values
            self._pl_dataset.set_data(robot_pos[0] + x_values, robot_pos[1] + y_values)
