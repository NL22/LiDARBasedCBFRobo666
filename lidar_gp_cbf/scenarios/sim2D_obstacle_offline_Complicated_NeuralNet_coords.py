    ...
    def compute_control(self, feedback, computed_control): # INITIALIZE CONTROLLER
        ...
        #for i in [2]:
            ...
            if obstacle == False:
                ...
                normalized_data=(localized_points - np.array([-0.5, -0.5])) / (np.array([0.5 - (-0.5), 0.5 - (-0.5)]))
                padded_edges = np.vstack([normalized_data, padding])
                flattened = padded_edges.reshape(-1)
                data_point = np.vstack([np.array([0.0,0.0]),padded_edges]).reshape(-1)
                alpha= torch.tensor([0.12])
                # Convert to tensor
