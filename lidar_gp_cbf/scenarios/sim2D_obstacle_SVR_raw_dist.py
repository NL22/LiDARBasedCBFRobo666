...
if PYSIM:
    ...
    from control_lib.SVM_raw_dist_h import OnlineSVMModel
        ...
        #for i in [2]:
            ...
            for j in range(360):
                if sensing_data[j] < SceneSetup.sense_dist: 
                    ...
                    self.svm[i].set_new_data(new_X=distance, theta=theta_obstacle)
            ...
            if self.svm[i].N < 10:
                ...
                theta_robot_cbf = feedback.get_robot_i_theta(i)  # Get robot's theta
                distance = np.array([[np.linalg.norm(current_data_X),theta_robot_cbf[0]]])
                svm_G, svm_h, true_svm_h =self.svm[i].get_cbf_safety_prediction(distance) #Change iput to distance
