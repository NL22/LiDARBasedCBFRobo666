...
if PYSIM:
    ...
    from control_lib.SVM_dist_h import OnlineSVMModel
        ...
        #for i in [2]:
            ...
            for j in range(360):
                if sensing_data[j] < SceneSetup.sense_dist: 
                    ...
                    edge_data_X_local = np.reshape(np.array([x_obs_local, y_obs_local]), (1, 2))

                    # Pass the local coordinates to the SVM model
                    self.svm[i].set_new_data(edge_data_X_local, sense_dist=SceneSetup.sense_dist, rob_pos=current_q)
